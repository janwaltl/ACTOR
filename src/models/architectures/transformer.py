...
def subseq_mask(n):
    """Attent to past and current context"""
    mask = (torch.triu(torch.ones(n, n)) == 0).transpose(0, 1)
    return mask
...
class Encoder_TRANSFORMER(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.ablation == "average_encoder":
            ...
            xseq = torch.cat(
                [self.muQuery[y][None], self.sigmaQuery[y][None], x], dim=0
            )
...
class Decoder_TRANSFORMER(nn.Module):
    def __init__(
        ...
        **kargs
        ...
        if USE_DECODER_ATTN:
            self.register_buffer("attn_mask", subseq_mask(num_frames + 1))
    ...
    def forward(self, batch):
        ...
        if USE_DECODER_ATTN:
            attn = self.attn_mask[:nframes, :nframes]
        else:
            attn = None
        ...
        output = self.seqTransDecoder(
            tgt=timequeries, memory=z, tgt_mask=attn, tgt_key_padding_mask=~mask
        )
